...
export interface Env {
  ...
  OPENAI_JD_MODEL?: string
  OPENAI_LINKEDIN_MODEL?: string
  ...
  // Also support README-style credentials
  CEIPAL_EMAIL?: string
  CEIPAL_API_KEY?: string
  CEIPAL_PASSWORD: string
  CEIPAL_BASE_URL?: string
  CEIPAL_ACCESS_TOKEN?: string
  CEIPAL_REFRESH_TOKEN?: string
  ...
  // Email token secret (for verify/reset/invite)
...
function readRaw(): Partial<Env> {
  ...
  return {
    ...
    OPENAI_RANKING_MODEL: env.OPENAI_RANKING_MODEL,
    OPENAI_JD_MODEL: env.OPENAI_JD_MODEL,
    OPENAI_LINKEDIN_MODEL: env.OPENAI_LINKEDIN_MODEL,

    LLAMAINDEX_API_KEY: env.LLAMAINDEX_API_KEY,

    LANDING_API_KEY: env.LANDING_API_KEY,

    CEIPAL_CLIENT_ID: env.CEIPAL_CLIENT_ID,
    CEIPAL_CLIENT_SECRET: env.CEIPAL_CLIENT_SECRET,
    CEIPAL_USERNAME: env.CEIPAL_USERNAME,
    CEIPAL_EMAIL: env.CEIPAL_EMAIL,
    CEIPAL_API_KEY: env.CEIPAL_API_KEY,
    CEIPAL_PASSWORD: env.CEIPAL_PASSWORD,
    CEIPAL_BASE_URL: env.CEIPAL_BASE_URL,
    CEIPAL_ACCESS_TOKEN: env.CEIPAL_ACCESS_TOKEN,
    CEIPAL_REFRESH_TOKEN: env.CEIPAL_REFRESH_TOKEN,
    ...
    RUN_ACTOR_FETCH_DATA: env.RUN_ACTOR_FETCH_DATA,
  }
}
...
export function validateEnv(requiredKeys: EnvKey[]): EnvValidationResult {
  ...
  if (raw.WHATSAPP_PROVIDER === 'meta') {
    ;(['META_WHATSAPP_ACCESS_TOKEN', 'META_WHATSAPP_PHONE_NUMBER_ID'] as EnvKey[]).forEach((k) => {
      if (!raw[k]) missing.push(k)
    })
  }
  if (raw.WHATSAPP_PROVIDER === 'twilio') {
    ;(['TWILIO_ACCOUNT_SID', 'TWILIO_AUTH_TOKEN', 'TWILIO_WHATSAPP_FROM'] as EnvKey[]).forEach((k) => {
      if (!raw[k]) missing.push(k)
    })
  }

  // Basic format checks
  if (raw.RESEND_FROM_EMAIL && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(raw.RESEND_FROM_EMAIL)) {
    problems.push('RESEND_FROM_EMAIL must be a valid email')
  }
